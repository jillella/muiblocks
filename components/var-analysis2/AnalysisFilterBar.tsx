...
const comparisonWeeks = ['Prior week', 'Prior 2 weeks', 'Prior month', 'Prior quarter'];
...
export default function AnalysisFilterBar() {
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Stack direction="row" spacing={2} useFlexGap sx={{ flexWrap: { xs: 'wrap', md: 'nowrap' }, pb: 0.5 }}>
        <CobPicker />

        <FormControl sx={{ flex: { xs: '1 1 100%', md: '1 1 0' }, minWidth: { md: 220 } }}>
          <FilterSelect
            id="scenario-type-select-2"
            value={scenarioType}
        ...
        <FormControl sx={{ flex: { xs: '1 1 100%', md: '1 1 0' }, minWidth: { md: 220 } }}>
          <FilterSelect
            id="comparison-week-select"
            value={comparison}
            onChange={(event: SelectChangeEvent<unknown>) => setComparison(event.target.value as string)}
            displayEmpty
            renderValue={(selected) => (selected as string) || 'Compare To'}
            MenuProps={{ PaperProps: { sx: menuPaperSx }, MenuListProps: { sx: { py: 0.5 } } }}
          >
            {comparisonWeeks.map((option) => (
              <MenuItem key={option} value={option} sx={{ minHeight: 46, fontSize: '0.95rem' }}>
                {option}
              </MenuItem>
            ))}
